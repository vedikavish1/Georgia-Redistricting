...
"""
"This document runs short bursts on Georgia's block group data. Since these bursts ran faster than the bursts on precincts data, 
this document is predominantly used in data-gathering. First, the ideal population of each district is calculated, after 
which the initial partition is created. This is the seed plan from which each short burst starts. Then, the short bursts are run 
according to the specified parametres (burst length and number of bursts), recording the maximum number of majority-minority 
districts observed in each burst and the ensemble of bursts. 
"""
...
# Import the Georgia block groups graph: 
georgia_bg = pickle.load(open("GA_blockgroup_graph.p", "rb"))
...
"""
pop = 0
for v in range(len(georgia_bg.nodes())):
    totpop = georgia_bg.nodes[v]["TOTPOP10"]
    pop = pop + totpop 
...
The relevant graph is the Georgia block groups graph. The recursive_tree_part function is used for the assignment argument, 
...
initial_partition = Partition(georgia_bg, 
assignment = recursive_tree_part(georgia_bg, range(num_dist), ideal_pop, "TOTPOP10", 0.02, 10), 
updaters={
    "cut edges": cut_edges, 
    "connectedness": (nx.is_connected(georgia_bg)), 
    "population": Tally("TOTPOP10", alias = "population"), 
    "BVAP": Tally("BVAP10", alias = "BVAP"), 
    "VAP": Tally("VAP10", alias = "VAP")
})


## Check that each partition has information saved:
for district, BVAP in initial_partition["BVAP"].items():
    print("District{}: {}". format(district, BVAP))
for district, VAP in initial_partition["VAP"].items():
    print("District{}: {}". format(district, VAP))
...
num_bursts = 1000  # number of bursts in the run
...
proposal1 = partial(recom, pop_col = "TOTPOP10", pop_target = ideal_pop, epsilon = 0.02, node_repeats = 1)
